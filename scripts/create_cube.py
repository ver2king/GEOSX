...
#         for j in range(ny):
#             for i in range(nx):
#                 p0 = points.index((i + nx + 1, j, k))
#                 p1 = points.index((i + 1 + nx + 1, j, k))
#                 p2 = points.index((i + 1 + nx + 1, j + 1, k))
#                 p3 = points.index((i + nx + 1, j + 1, k))
#                 p4 = points.index((i + nx + 1, j, k + 1))
#                 p5 = points.index((i + 1 + nx + 1, j, k + 1))
#                 p6 = points.index((i + 1 + nx + 1, j + 1, k + 1))
#                 p7 = points.index((i + nx + 1, j + 1, k + 1))
#                 c = (p0, p1, p2, p3, p4, p5, p6, p7)
#                 cells.append(c)
#     return cells


def create_cells_2(nx, ny, nz, points):
    cells = []
    for k in range(nz):
        for j in range(ny):
            for i in range(nx + 1):
...
#     tmp = p[0] - 1 if p[0] > nx else p[0]
#     return tmp, p[1], p[2]


def coords_point_2(nx, ny, nz, p):
    tmp = p[0] - 1 if p[0] > (nx // 2) else p[0]
    return tmp, p[1], p[2]


# def print_vtk(nx, ny, nz, points, cells):
...
#     print("")
#     print("CELLS " + str(len(cells)) + " " + str(len(cells) * (8 + 1)))
#     for c in cells:
#         print("8 " + " ".join([str(pi) for pi in c]))
#     print("")
#     print("CELL_TYPES " + str(len(cells)))
#     for c in cells:
#         print("12")


def print_vtk_2(nx, ny, nz, points, faces, cells):
    print("# vtk DataFile Version 2.0")
    print("vtk output")
    print("ASCII")
    print("DATASET UNSTRUCTURED_GRID")
...
def main():
    ...
    nx, ny, nz = 10, 10, 1
    assert nx % 2 == 0
    points = create_points_2(nx, ny, nz)
    assert (len(points) == 121 * 2 + 11 * 2)
    cells = create_cells_2(nx, ny, nz, points)
    assert (len(cells) == 100)
    faces = create_faces(nx, ny, nz, points)
    assert (len(faces) == ny)
    print_vtk_2(nx, ny, nz, points, faces, cells)

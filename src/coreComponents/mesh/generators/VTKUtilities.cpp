                   ...
                   integer const useGlobalIds )
{
  ...
  else if( useGlobalIds >= 0 && globalIdsAvailable )
  {
    ...
    GEOSX_ERROR_IF( globalCellId->GetNumberOfComponents() != 1 && globalCellId->GetNumberOfTuples() != output->GetNumberOfCells(),
                    "Global cell IDs are invalid. Check the array or enable automatic generation (useGlobalId < 0)" );
    GEOSX_ERROR_IF( globalPointId->GetNumberOfComponents() != 1 && globalPointId->GetNumberOfTuples() != output->GetNumberOfPoints(),
                    "Global cell IDs are invalid. Check the array or enable automatic generation (useGlobalId < 0)" );
